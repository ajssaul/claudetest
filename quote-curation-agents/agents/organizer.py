...
수정 규칙:
- ⚠️ 최우선: 1명언 = 1주제. 다른 주제 문장이 섞여 있으면 제거. 5문장 채우려고 다른 주제 문장 넣지 마라
- 번역은 '-다' 체, 성숙한 어투
...
0. ⚠️ 최우선 규칙 — 1명언 = 1주제:
   - 1개 명언 안에 2개 이상 주제가 섞이면 절대 안 된다
   - 원문에 A-A-B-B-A처럼 다른 주제가 끼어 있으면, 해당 문장을 제거하고 1주제만 남겨라
   - 5~6문장을 채우기 위해 다른 주제의 문장을 억지로 포함하지 마라
   - 1주제를 유지하면서 5문장이 안 되면, 3~4문장이라도 1주제가 낫다

1. 원문 길이:
   - 목표: 5~6문장이지만, 1명언 1주제 유지가 우선
   - 1주제로 5~6문장 가능하면 → 5~6문장
   - 1주제로 5문장 어려우면 → 3~4문장으로 축소 (1주제 유지가 더 중요)
   - 비퍼블릭 도메인 도서: 최대 2문장
   - ⚠️ 각 명언을 읽고 "이 명언은 하나의 주제만 다루고 있는가?" 를 반드시 확인해라
...
- ⚠️ 1개 명언이 1개 주제만 다루는가? (다른 주제 문장이 섞여 있지 않은가?)
- 원문 문장 수가 기준에 맞는가? (1주제 유지가 안 되면 5문장 미만도 허용)
- 번역이 모두 '-다' 체인가?
